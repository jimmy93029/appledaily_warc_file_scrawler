...
from extract_text import valid_text, extract_text, has_chinese
from extract_image2 import valid_image, extract_image_info, get_image_filename
from utils_ninjs3 import get_new_id, meta_data, write_json
...
def process_warc(file_path, output_base_dir="/mnt/data/datastw"):
    ...
    print(f"Directory created or exists: {dirr}")
    ...
    with open(file_path, 'rb') as file:
        ...
        for record in warc_iter:
            ...
            if record.is_http and record.http_headers:
                Content_type = record.http_headers.get('Content-Type', None)
        
                if valid_text(Content_type, uri):
                    try:
                    ...
                    # Use WARC date from the record if available
                    ...
                    if not has_chinese(content) or not title:
                        continue
